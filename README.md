<h1>Результаты тестов (50к тестовых запросов)</h1>
С одной ошибкой  
...
<hr>
<h3>Что мы делали последнюю неделю</h3>
Мы решили собрать побольше данных для обучения и теста спеллера, так как при тестировании на данных, на которых мы не обучали, точность значительно понизилась.<br> Мы собрали больше данных, почистили их как смогли и сделали их новыми обучающими данными. В итоге контекст стал отрабатывать хуже так как поменялся баланс слов в обучающей выборке.<br>Смогли поправить немного изменив систему штрафов за слова, которые далеки от предполагаемых. Также убрали короткие слова (в основном предлоги), тем самым обратно подняли скор до приемлемых процентов.<br>Прогнали тесты по памяти скорости и точности в сравнении с нашим спеллером, тем который нашли в гитлабе и яндексовским. Итог привели в сравнительную таблицу.
<hr>
<h3>Комментарии</h3>
<p><i>Токенайзер</i></p>
